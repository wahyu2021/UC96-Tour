...
export function RegistrationForm({
  ...
  const onSubmit = async (data: TeamRegistrationInput) => {
    ...
    try {
      ...
      if (masterTeam) {
        ...
        res = await fetch('/api/player/register', {
          method: 'POST',
      ...
      toast.success(result.message || 'Berhasil terdaftar!', { id: toastId });
  ...
  const logoUrlWatcher = useWatch({ control, name: 'logoUrl' });
    ...
    <div className="w-full max-w-3xl rounded-xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-10 dark:border-neutral-800 dark:bg-[#1e1e1e]">
      ...
      {masterTeam ? (
        <form
          ...
          className="animate-in fade-in block space-y-6 duration-500"
        >
          <div className="mb-6 rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-900/20">
            <h3 className="font-semibold text-green-800 dark:text-green-300">
              Mode Pendaftaran 1-Klik Aktif
            </h3>
            <p className="mt-1 text-sm text-green-700 dark:text-green-400">
              Anda akan mendaftarkan tim utama{' '}
              <strong>
                [{masterTeam.tag}] {masterTeam.name}
              </strong>{' '}
              dengan {masterTeam.players.length} pemain.
            </p>
          </div>
          <div>
                ...
                <div className="flex items-center gap-5 rounded-md border border-dashed border-neutral-300 p-4 dark:border-neutral-700 dark:bg-[#121212]">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mt-2 text-xs text-neutral-500">
                      Gunakan file JPG, PNG, atau WEBP beresolusi tinggi
                      (Maksimal 5MB).
                    </p>
                  </div>
                </div>
