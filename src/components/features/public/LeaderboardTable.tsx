...
import { Trophy, RefreshCw, Crown, Medal, Crosshair, Flame } from 'lucide-react';
...
export function LeaderboardTable() {
  ...
  const getPodiumCards = () => {
    ...
    return (
      <div className="mb-16 mt-10 flex flex-col items-center justify-center gap-6 sm:flex-row sm:items-end sm:gap-4 md:gap-8">
        {arranged.map((team, idx) => {
          ...
          const containerClasses = cn(
            ...
            isFirst && "z-10 bg-gradient-to-b from-yellow-50 to-white dark:from-yellow-900/20 dark:to-[#121212] border-yellow-300 dark:border-yellow-600/50 sm:-mt-8 shadow-yellow-500/20 dark:shadow-yellow-500/10",
            isSecond && "bg-gradient-to-b from-slate-50 to-white dark:from-slate-800/30 dark:to-[#121212] border-slate-300 dark:border-slate-600/50 shadow-slate-500/10",
            isThird && "bg-gradient-to-b from-amber-50 to-white dark:from-amber-900/20 dark:to-[#121212] border-amber-300 dark:border-amber-700/50 shadow-amber-500/10"
          );
          ...
          return (
            <Link 
              ...
              key={team.teamId}
              className={cn(containerClasses, "animate-in slide-in-from-bottom-8 fade-in")}
              style={{ animationDelay: `${(idx + 1) * 150}ms`, animationFillMode: 'both' }}
            >
              <div className={badgeClasses}>
                {isFirst ? <Crown className="h-6 w-6" /> : <Medal className="h-6 w-6" />}
              </div>
              ...
              <div className="my-4 flex items-center gap-3 rounded-xl bg-white/60 px-4 py-2 dark:bg-black/20">
                <div className="text-center">
                  <p className="text-[10px] font-bold text-neutral-500 uppercase">Kills</p>
                  <p className="text-sm font-black text-red-600 dark:text-red-400">{team.totalKills}</p>
                </div>
                <div className="h-6 w-px bg-neutral-200 dark:bg-neutral-800"></div>
                <div className="text-center">
                  <p className="text-[10px] font-bold text-neutral-500 uppercase">WWCD</p>
                  <p className="text-sm font-black text-brand-600 dark:text-brand-400">{team.wwcd}</p>
                </div>
              </div>

              <div className="flex w-full flex-col items-center rounded-xl bg-neutral-900 px-4 py-3 dark:bg-neutral-800">
                <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest">Total Poin</p>
                <p className={cn(
                  "text-3xl font-black",
      ...
      <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
            Peringkat tim berdasarkan total akumulasi poin eliminasi dan posisi akhir.
          </p>
        </div>
        <Button
          variant="outline"
        ...
        >
          <RefreshCw
            className={cn("mr-2 h-4 w-4 text-brand-500", isLoading && "animate-spin")}
          />
      ...
      {isLoading ? (
        <div className="flex h-64 items-center justify-center rounded-2xl border border-neutral-200 bg-white/50 dark:border-neutral-800 dark:bg-[#121212]/50">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm font-medium text-neutral-500">Menganalisis data pertandingan...</p>
          </div>
        </div>
      ) : data.length === 0 ? (
        <div className="flex h-64 items-center justify-center rounded-2xl border border-neutral-200 bg-white/50 dark:border-neutral-800 dark:bg-[#121212]/50">
          <p className="text-center text-neutral-500">Belum ada tim atau data skor pertandingan.</p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          {restOfTeams.length > 0 && (
            <div className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm sm:p-6 dark:border-neutral-800 dark:bg-[#121212]">
              <h3 className="mb-4 text-sm font-bold text-neutral-500 uppercase tracking-widest dark:text-neutral-400">
                Peringkat Tim Lainnya
              </h3>
              <div className="flex flex-col gap-3">
                {/* Header Kolom Desktop */}
                <div className="hidden grid-cols-12 gap-4 px-4 py-2 text-xs font-bold text-neutral-400 uppercase tracking-wider sm:grid">
                ...
                {restOfTeams.map((team, idx) => (
                  <Link
                    ...
                    style={{ animationDelay: `${(idx + 1) * 50}ms`, animationFillMode: 'both' }}
                  >
                    {/* Rank */}
                    <div className="col-span-1 flex w-full justify-between sm:block sm:w-auto sm:text-center">
                      <span className="text-xs font-bold text-neutral-400 sm:hidden uppercase">Rank</span>
                    ...
                    <div className="col-span-5 flex w-full items-center gap-3">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-neutral-200 font-bold text-neutral-500 dark:bg-neutral-800">
                        {team.logoUrl ? (
                          ...
                          <img src={team.logoUrl} alt={team.teamName} className="h-full w-full rounded-lg object-cover" />
                        ) : (
                          team.teamName.substring(0, 2).toUpperCase()
                        )}
                      </div>
                      <div className="flex flex-col">
                        <span className="font-bold text-neutral-900 group-hover:text-[var(--color-primary)] dark:text-white transition-colors">
                          {team.teamName}
                        </span>
                    ...
                    <div className="col-span-6 flex w-full justify-between sm:contents">
                      <div className="col-span-2 flex flex-col sm:items-center">
                        <span className="text-[10px] font-bold text-neutral-400 sm:hidden uppercase">Kills</span>
                      ...
                      <div className="col-span-2 flex flex-col sm:items-center">
                        <span className="text-[10px] font-bold text-neutral-400 sm:hidden uppercase">WWCD</span>
                        {team.wwcd > 0 ? (
                      ...
                      <div className="col-span-2 flex flex-col items-end sm:items-end">
                        <span className="text-[10px] font-bold text-neutral-400 sm:hidden uppercase">Total</span>
                        <span className="font-heading text-xl font-black text-brand-600 dark:text-brand-400">
                          {team.totalPoints}
                        </span>
                      </div>
                    </div>
