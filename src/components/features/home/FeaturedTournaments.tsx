    ...
    <section className="w-full bg-[#0a0a0a] py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-10 flex items-end justify-between">
          <div>
            <h2 className="font-heading text-3xl font-black text-white uppercase italic md:text-4xl">
              Turnamen{' '}
              <span className="text-[var(--color-primary)]">Sorotan</span>
            </h2>
            <p className="mt-2 text-neutral-400">
              Kompetisi yang sedang atau akan segera berlangsung.
            </p>
          </div>
          <Link
            href="/tournaments"
            className="hidden text-sm font-bold text-neutral-300 hover:text-white md:block"
          >
            Lihat Semua &rarr;
          </Link>
        </div>
        ...
        {!tournaments || tournaments.length === 0 ? (
          ...
          <div className="flex w-full snap-x snap-mandatory gap-6 overflow-x-auto pt-4 pb-8">
            {tournaments.map((tour) => (
              <Link
                key={tour.id}
                href={`/tournaments/${tour.id}`}
                className="group relative flex w-80 shrink-0 snap-center flex-col overflow-hidden rounded-2xl border border-neutral-800 bg-[#121212] transition-transform hover:-translate-y-2 hover:shadow-[var(--color-primary)]/20 hover:shadow-2xl"
              >
                {/* Image Section */}
                <div className="relative h-48 w-full bg-neutral-900">
                  ...
                  <div className="absolute top-4 left-4">
                    <Badge
                      variant={
                        tour.status === 'ONGOING' ? 'success' : 'warning'
                      }
                    >
                      {tour.status === 'ONGOING'
                        ? 'Sedang Berlangsung'
                ...
                <div className="flex flex-col p-6">
                  <h3 className="font-heading mb-4 line-clamp-2 text-xl leading-tight font-bold text-white group-hover:text-[var(--color-primary)]">
                    {tour.name}
                  </h3>
        ...
        <div className="mt-4 flex justify-center md:hidden">
          <Link
            href="/tournaments"
            className="rounded-full bg-neutral-800 px-6 py-2 text-sm font-bold text-white hover:bg-neutral-700"
          >
