...
export default async function TournamentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="w-full">
      {/* Hero Banner Section */}
      <div className="relative min-h-[40vh] w-full bg-neutral-900 overflow-hidden">
        {(tour as any).backgroundUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img 
            src={(tour as any).backgroundUrl} 
        ...
        <div className="relative mx-auto flex min-h-[40vh] max-w-7xl flex-col justify-end px-4 pb-12 sm:px-6 lg:px-8">
          <Badge variant={dynamicStatus === 'ONGOING' ? 'success' : 'default'} className="mb-4 w-fit px-3 py-1 font-bold">
            {dynamicStatus}
          </Badge>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-6 text-sm font-medium text-neutral-300">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>{tour.teams.length} / {tour.maxSlots} Tim Terdaftar</span>
            </div>
            {tour.prizePool && (
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-4 lg:gap-10">
          ...
          <div className="lg:col-span-1">
            {tour.bannerUrl ? (
              <div className="sticky top-24 overflow-hidden rounded-2xl shadow-xl border border-neutral-200 dark:border-neutral-800">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img 
                  src={tour.bannerUrl} 
                  alt={tour.name}
              ...
              <div className="sticky top-24 flex aspect-[9/16] w-full flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-300 bg-neutral-50 dark:border-neutral-800 dark:bg-[#121212]">
                ...
                <span className="mt-2 text-center text-xs font-bold text-neutral-500 uppercase tracking-widest px-4">NO POSTER</span>
              </div>
            )}
          </div>
            ...
            <section>
              ...
              <div className="prose prose-neutral dark:prose-invert max-w-none rounded-2xl border border-neutral-200 bg-white p-6 dark:border-neutral-800 dark:bg-[#121212]">
                {tour.description ? (
                  ...
                  <p className="text-neutral-500 italic">Tidak ada deskripsi spesifik untuk turnamen ini.</p>
                )}
              </div>
            </section>
          </div>
          ...
          <div className="lg:col-span-1">
            <h2 className="font-heading mb-6 flex items-center gap-2 text-xl font-bold text-neutral-900 dark:text-white">
              <Swords className="h-6 w-6 text-brand-500" />
              ...
              {tour.teams.length === 0 ? (
                ...
                tour.teams.map((team) => (
                  <Link
                    href={`/teams/${team.id}`}
                    key={team.id}
                    className="group flex items-center gap-4 rounded-xl border border-neutral-200 bg-white p-3 transition-colors hover:border-brand-500 hover:bg-neutral-50 dark:border-neutral-800 dark:bg-[#121212] dark:hover:border-brand-500/50 dark:hover:bg-neutral-900"
                  >
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-neutral-100 font-bold text-neutral-500 dark:bg-neutral-800">
                      {team.logoUrl ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={team.logoUrl} alt={team.name} className="h-full w-full rounded-lg object-cover" />
                      ) : (
                        team.tag
                      )}
                    </div>
                    <div>
                      <h4 className="font-bold text-neutral-900 group-hover:text-brand-600 dark:text-white dark:group-hover:text-brand-400">
                        {team.name}
                      </h4>
            ...
            {dynamicStatus !== 'COMPLETED' && tour.teams.length < tour.maxSlots && (
              <div className="mt-6">
                <Link
                  href="/register"
